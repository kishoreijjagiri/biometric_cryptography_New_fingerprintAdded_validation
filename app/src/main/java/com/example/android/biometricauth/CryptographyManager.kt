...
interface CryptographyManager {
    ...
    fun isBiomtricDataUpdated(keyName: String): Boolean
...
private class CryptographyManagerImpl : CryptographyManager {
    ...
    override fun isBiomtricDataUpdated(name :String): Boolean {
        return try {
            val secretKey = getOrCreateSecretKey(name)
            val cipher = getCipher()
            cipher.init(Cipher.ENCRYPT_MODE, secretKey)
            false
        }catch ( e : KeyPermanentlyInvalidatedException)
        {
            deleteExistingKey(name)
            true
        }
    ...
    private fun deleteExistingKey(keyName: String) {
        // If Secretkey was previously created for that keyName, then grab and return it.
        val keyStore = KeyStore.getInstance(ANDROID_KEYSTORE)
        keyStore.load(null) // Keystore must be loaded before it can be accessed
